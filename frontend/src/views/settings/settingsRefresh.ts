...
export const hydrateRefreshSettings = (source: Settings): RefreshSettings => {
  ...
  return {
    ...
    register_default_count: pickNumber(defaults.register_default_count || 20, current.register_default_count),
    auto_refresh_accounts_seconds: pickNumber(defaults.auto_refresh_accounts_seconds || 60, current.auto_refresh_accounts_seconds),
    scheduled_refresh_enabled: pickBoolean(defaults.scheduled_refresh_enabled || false, current.scheduled_refresh_enabled),
    scheduled_refresh_interval_minutes: pickNumber(defaults.scheduled_refresh_interval_minutes || 30, current.scheduled_refresh_interval_minutes),
    scheduled_refresh_cron: pickString(defaults.scheduled_refresh_cron || '', current.scheduled_refresh_cron),
    verification_code_resend_count: pickNumber(defaults.verification_code_resend_count || 2, current.verification_code_resend_count),
    refresh_batch_size: pickNumber(defaults.refresh_batch_size || 5, current.refresh_batch_size),
    refresh_batch_interval_minutes: pickNumber(defaults.refresh_batch_interval_minutes || 30, current.refresh_batch_interval_minutes),
    refresh_cooldown_hours: pickNumber(defaults.refresh_cooldown_hours || 12, current.refresh_cooldown_hours),
    delete_expired_accounts: pickBoolean(defaults.delete_expired_accounts || false, current.delete_expired_accounts),
    auto_register_enabled: pickBoolean(defaults.auto_register_enabled || false, current.auto_register_enabled),
